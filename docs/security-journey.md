...
This is unit and Chromium UI-contract evidence, not end-to-end native onboarding evidence. The Linux two-shell and packaged macOS scripts now begin from clean onboarding state, assert focused Welcome, and choose Explore before their existing flows, but the revised packaged journeys were not rerun locally. Neither script completes both first-run create and join, and the Playwright scenario does not prove relay admission, MLS processing, credential storage, or platform folder-picker behavior. Those operations retain the separately scoped production-path evidence above.
