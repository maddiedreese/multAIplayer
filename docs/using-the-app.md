...
On the official hosted relay, GitHub sign-in is required. Local-only branch/commit workflows without GitHub sign-in may be available only on local/LAN or self-hosted relays configured without GitHub auth. Push, draft PR creation, and GitHub Actions refreshes require native GitHub sign-in. The alpha requests `read:user repo`; GitHub's broad `repo` scope includes private repositories available to the signed-in account, not only the repository currently open in multAIplayer. Do not approve the device-code grant if that access is unacceptable. Signing out deletes the Keychain token and relay session, while revoking the OAuth grant is a separate action in GitHub's application settings.
