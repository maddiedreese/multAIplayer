...
On the official hosted relay, GitHub identity sign-in is required for workspace membership and invitations. Identity verification itself uses `read:user`; it does not need repository authority. The current alpha nevertheless requests one combined `read:user repo` device-code grant because optional draft PR creation and GitHub Actions refreshes use the token directly from native Rust. Git push is separate and uses the active host's ordinary Git credential path. GitHub's broad `repo` scope includes private repositories available to the signed-in account, not only the repository currently open in multAIplayer. This is a grant-boundary limitation, not an identity requirement: the current UI cannot request identity-only access first and add repository access later. Do not approve the device-code grant if that access is unacceptable. Signing out deletes the Keychain token and relay session, while revoking the OAuth grant is a separate action in GitHub's application settings.
