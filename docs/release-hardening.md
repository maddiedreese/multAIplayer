...
When an alpha tester hits an ordinary crash or app bug, ask them to open Account settings and click `Copy diagnostics`. The native app persists capture-redacted warning/error entries as `diagnostics.jsonl` in the platform app log directory (`~/Library/Logs/com.multaiplayer.desktop/` on macOS), with `0600` permissions and seven-day, 256 KiB, and 500-entry limits. The web preview remains memory-only. The export path validates each stored line and re-redacts it before inclusion; no generic diagnostic read command should be added.

The bundle contains app version, runtime/platform metadata, relay origins, and recent redacted warning/error entries. It does not intentionally include room transcripts, room secrets, terminal output, browser contents, file contents, invite fragments, or GitHub tokens. Users should still review the copied JSON before attaching it to a GitHub issue. Diagnostics are not encrypted at rest in this alpha; revisit that decision before collecting richer data. During review, require stable error codes and bounded ids instead of payload objects even when the object-key denylist would omit known sensitive fields.
