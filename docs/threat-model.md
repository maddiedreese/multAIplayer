...
Terminal output and received terminal command requests are scanned before sharing or host approval. Content that appears to dump environment variables, read `.env` or credential files, use a recognizable network operation, or include token-like text shows a warning. These text classifiers are review aids, not denylists or complete capability/secret detectors. Interpreters, scripts, aliases, hooks, configuration, and arbitrary executables can access files or the network without recognizable command text. A classifier match makes native approval one-shot; absence of a match does not make a command safe. Codex app-server command approvals always show the same incomplete-analysis warning, including when no recognizable marker is present. Structured Codex network permissions and every filesystem permission outside the canonical active project root are denied; in-root filesystem permissions are decided structurally, not by credential-name substring. Codex/model output, webpages, attachments, pasted text, rendered content, relay peers, and the webview are all untrusted command sources. Remote room-command authority is enforced outside the webview: a native operating-system dialog approves the exact room, canonical working directory, command, and execution kind before the sandboxed process spawns, and Rust consumes the resulting short-lived authorization once. Room or workspace substitution, command substitution, replay, expiry, cancellation, concurrent prompt attempts, and calls without native authorization fail closed. Executable-name allowlists are not a security boundary because tools such as Git and npm can execute hooks, configuration, and scripts. Approved remote commands run as the host account under a macOS profile that confines filesystem writes to the canonical project and limits reads to that project plus named system/toolchain locations. Child processes, inherited environment, project hooks and configuration, and network access remain available, so this is project-filesystem confinement rather than complete host isolation.

The interactive terminal is a separate local-user surface. Clicking **New terminal** starts a PTY directly in the canonical room project and input goes straight to that already-running PTY without native confirmation dialogs. This preserves ordinary terminal job control and lets the local user navigate outside the initial folder. It also means a compromise of the trusted main Tauri webview could write to a terminal session that the local user already opened. Room membership alone does not expose that input capability to a remote peer; remote commands use the reviewed flow above. The trusted terminal surface receives the live PTY screen, including ordinary process output and shell echo, in memory so it can behave like an external terminal; this live screen is excluded from retained room history. A separate bounded native-redacted line projection is used for retention, Markdown export, risk classification, and explicit sharing. Direct local terminal execution otherwise has the host account's ordinary shell authority.

Approval fatigue and prompt injection are primary product-security risks. Native dialogs are reserved for remote room commands and other actual boundary crossings rather than ordinary local terminal keystrokes. Each remote-command dialog displays the exact native operation. For repeated one-shot room commands only, the native dialog offers “Repeat this command text for 10 minutes” and warns that workspace files, scripts, hooks, configuration, and environment may change between runs. The in-memory grant binds the exact room, canonical workspace, and command bytes. Rust alone stores, matches, expires, and revokes these grants; app restart clears them, the terminal-panel revoke action requires a separate native confirmation, and they never authorize PTY creation or input. There is no executable-name or command-family allowlist.
