...
`MULTAIPLAYER_RELAY_MAX_DURABLE_ENTRIES` bounds the combined top-level durable records and team-member records held by one process (default `250000`). `MULTAIPLAYER_RELAY_MAX_DURABLE_ENTRIES_PER_TEAM` independently caps team-owned records (default `25000`), so one team cannot consume the global ceiling. Startup fails when loaded state exceeds either ceiling. New compound insertions reserve quota and then either complete or roll back every contributed entity and quota record when capacity rejects a later insertion; capacity rejection does not poison a healthy database. Attachment memory is bounded separately by actual stored ciphertext bytes, not the client-declared plaintext size. This is a deliberate alpha storage model, not a claim of a relational query layer or an unbounded dataset.
