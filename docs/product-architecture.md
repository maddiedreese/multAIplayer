...
If a room is forgotten on the device or relay membership is revoked, the desktop treats it as locally locked. While locked, the app blocks host claiming, host handoff acceptance, invite generation, invite approval decisions, terminal request decisions, room-key rotation, and model switching until the room is unlocked with a fresh invite or key.
