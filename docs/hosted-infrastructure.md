...
The official relay deploys the public `apps/relay` container to Railway with operator-managed persistence, backup retention, origin/rate-limit, and monitoring configuration. The official native desktop is compiled separately with the public GitHub Device Flow client id and exact relay origin. Secrets and production data are not repository content. [Self-hosting](self-hosting.md) documents the public deployment contract, while [Relay operations](relay-operations.md) covers public operator procedures; neither implies access to the hosted operator account.
