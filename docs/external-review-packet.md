...
- Focused desktop tests separately cover versioned onboarding normalization/migration, scope-reset invariants, storage failure and corruption recovery, readiness projection and raw-error suppression, the create-team/room retry checkpoint, production-invite adapter delegation and blocking approval-pending behavior, account readiness, and rendered component interaction/accessibility contracts. A full native first-run create/join journey is not evidence in this packet. The Linux two-shell and packaged macOS scripts assert clean-profile Welcome/focus/Explore before their existing flows; Linux also proves that a real protected admission request opens blocking device-verification guidance and accurately names active-host approval before the inspector denial flow resumes. This is native transition and recovery evidence, not submission through the onboarding invite form or complete native create/join coverage.
