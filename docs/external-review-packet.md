...
- Focused desktop tests separately cover versioned onboarding normalization/migration, scope-reset invariants, storage failure and corruption recovery, readiness projection and raw-error suppression, the create-team/room retry checkpoint, production-invite adapter delegation and blocking approval-pending behavior, account readiness, and rendered component interaction/accessibility contracts. A full native first-run create/join journey is not yet evidence in this packet. The Linux two-shell and packaged macOS scripts now assert the clean-profile Welcome/focus/Explore entry before their existing flows, but those revised packaged journeys were not rerun locally for this packet; their existing native claims above do not imply verified end-to-end onboarding.
