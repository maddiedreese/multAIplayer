...
The relay uses GitHub device-code OAuth to identify users and scope team and room metadata. The same relay-side session can support draft pull request creation and GitHub Actions status without exposing the access token to desktop clients. The default hosted scope is intended for identity and public-repository workflows. Self-hosters must explicitly choose the broader repository scope if they want private-repository GitHub operations.
