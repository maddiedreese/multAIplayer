...
A self-built desktop pins its public OAuth client id and exact relay HTTPS origin at compile time with `MULTAIPLAYER_NATIVE_GITHUB_CLIENT_ID` and `MULTAIPLAYER_NATIVE_RELAY_HTTP_ORIGIN`. No client secret is used. The current native grant is fixed to `read:user repo`: `read:user` supports relay identity verification, while `repo` supports optional pull-request and Actions API workflows and is not part of relay authority. Git push uses ordinary host Git credentials. Changing or splitting that grant requires a deliberately reviewed native build rather than a relay environment change.
...
Device ids are bound to one MLS key pair while registered. An authenticated user can retire an owned device from **Profile → Manage registered devices**, or with `DELETE /devices/:deviceId` and JSON `{ "confirmation": "my-device-id" }`. The exact signed-in GitHub account and matching target id are required; device proof is deliberately not required because this is the recovery path for lost local keys. Retirement is blocked while the device hosts a non-deleted room. It revokes device sessions and pending challenges, removes unused KeyPackages and invite artifacts directly bound to that device, revokes any invite already approved for it, and closes joined relay sockets.

Retirement is an explicit account-authorized key reset, not a recovery of the old MLS identity. The same device id can be registered later, but a different key produces the existing changed-fingerprint warning for peers that previously compared that device. Prefer a fresh device id when rotating keys, then rejoin rooms through the normal enrollment flow. Retirement does not recover old MLS state or history.
...
No client secret is embedded or required. The native alpha requests one combined `read:user repo` grant. The relay identity path uses `read:user`; optional native pull-request and Actions API workflows use `repo`. Git push uses the host's ordinary Git credential path, not the OAuth token. Repository authority is not part of relay membership or invitation authorization, but the current UI cannot request identity-only access and escalate later. GitHub's `repo` scope is intentionally broad: approving sign-in grants the native app access to private repositories available to that GitHub account, not only the repository currently open in multAIplayer. Do not approve the device-code grant if that access is unacceptable. The grant can be revoked independently from GitHub's application settings; signing out of multAIplayer deletes local token custody and the relay session but does not revoke the GitHub grant.
