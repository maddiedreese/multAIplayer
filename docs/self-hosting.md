...
If an existing relay database is unreadable, has missing or unsupported version metadata, contains malformed critical rows, or exceeds a configured startup ceiling, startup fails rather than serving an empty replacement workspace. Unreadable or unsupported evidence is quarantined beside the configured path with a `.corrupt-...` suffix where possible; operators must inspect the startup error and recover or restore the database deliberately. A ceiling violation leaves the database in place so the operator can adjust a verified configuration or reduce state offline. Keep regular backups of `MULTAIPLAYER_RELAY_DATA_PATH`; quarantine is a recovery aid, not a replacement for backups.
