...
The relay exposes content-free operational counters and fixed-bucket latency histograms in Prometheus text format at `/metrics`. Histograms cover publish queue through successful persistence/fanout, WebSocket send callback completion/backpressure, and synchronous SQLite writes, with standard `_bucket`, `_sum`, and `_count` series in seconds. Counters include active sockets, live sealed-blob count and bytes, published opaque-message count, accepted attachment upload count and bytes, upload rejection counts by reason, rate-limit and quota rejections, WebSocket connection outcomes, start time, and uptime. The endpoint is always bearer-authenticated and remains disabled until a token of at least 32 characters is configured:
