...
Origin entries are normalized to bare origins by the relay. `https://multaiplayer.com/` becomes `https://multaiplayer.com`. The exact `tauri://localhost` origin is also supported for the packaged macOS webview. Other custom schemes, paths, queries, fragments, credentials, wildcards, or custom ports on that desktop origin are rejected consistently by runtime startup and the production doctor. Native desktop requests without an `Origin` header are still allowed.
