...
The deletion ledger supports an S3-compatible bucket and a filesystem backend. Production should use a bucket outside the primary SQLite volume and every backup set that can restore it. The filesystem backend is suitable only when its directory is a separately protected mount excluded from primary backups. Each append-only immutable object contains a version, HMAC-SHA-256 pseudonym of the GitHub user id, deletion-attempt time, last-covered-backup expiry (`protectUntil`), random object id component, and an authentication MAC under a dedicated key. It contains no raw GitHub id, login, token, or deleted-record inventory. Repeated requests are safe and may create multiple objects for one pseudonym. When startup reconciliation finds identity data to remove, it writes a fresh object before committing primary cleanup, so a delayed retry covers backups made after the original request. The HMAC key must be separate from the session secret and bucket credentials.
...
Every startup purges tombstones whose individual protection horizons have elapsed, lists and authenticates every remaining object, hashes every GitHub identity found in primary state, and reapplies matching deletions even if SQLite says that tombstone was previously applied. That last comparison protects partial/manual restores that retain a local marker while resurrecting identity rows. Before that cleanup commit it appends a new tombstone protected from the cleanup attempt time. The same pass removes expired local pseudonymous markers only after the external list confirms their tombstones are gone. Any active tombstone blocks new GitHub sign-in for that identity; sign-in becomes possible again only after the newest protection horizon and successful purge.
