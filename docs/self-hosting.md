...
Operational verification on 2026-07-15 confirmed the live Railway production manifest uses `/readyz`, `restartPolicyType: ALWAYS`, one replica, and a 5 GB `/data` volume with about 92 MB used. Workspace notification rules for **Deployment Crashed** and **Deployment Oom Killed** cover all projects with email and in-app delivery. The poison path's forced nonzero exit therefore reaches both Railway restart supervision and the existing crash notification; the exact `relay_store_persistence_poisoned` record remains the diagnostic log signal. The same inspection confirmed that the prior relay deployment was skipped because the GitHub Release workflow failed, which is why the release-please bootstrap/tag validation is now part of `check:release-versions`.
...
`MULTAIPLAYER_RELAY_SQLITE_WAL_AUTOCHECKPOINT_PAGES` defaults to SQLite's `1000`. A representative local 250-page comparison reduced maximum observed event-loop delay and WAL size, but request and publish p99 regressed, so the shorter checkpoint was not adopted as the default. Keep synchronous persistence; use a longer hosted request/event-loop/SQLite-write measurement to decide whether checkpoint tuning is warranted. The exact workload and both results are recorded in [Relay synchronous-persistence soak](benchmarks/relay-soak-2026-07-15.md).

The relay still hydrates durable state into one process-local store at startup. `MULTAIPLAYER_RELAY_MAX_DURABLE_ENTRIES` bounds all top-level durable records plus individual team-member records; `MULTAIPLAYER_RELAY_MAX_DURABLE_ENTRIES_PER_TEAM` caps team-owned records below that global ceiling so one tenant cannot exhaust it. Reaching either aborts startup or rejects the insertion explicitly. Quota-consuming room and attachment insertions roll back their contributed quota/entity state when a later capacity claim fails. Lower the ceilings for constrained hosts, or shard whole teams across independent relays; do not raise them past measured memory capacity. Run one relay writer per SQLite database; general multi-instance coordination is not claimed until reads and all compare-and-swap mutations move behind a shared database service. Horizontal deployments also require shared rate limiting and attachment-storage coordination.
...
The relay exposes content-free operational counters, event-loop-delay gauges, and fixed-bucket latency histograms in Prometheus text format at `/metrics`. Histograms cover publish queue through successful persistence/fanout, WebSocket send callback completion/backpressure, and synchronous SQLite writes, with standard `_bucket`, `_sum`, and `_count` series in seconds. Counters include active sockets, live sealed-blob count and actual stored ciphertext bytes, published opaque-message count, accepted attachment upload count and stored bytes, upload rejection counts by reason, rate-limit and quota rejections, WebSocket connection outcomes, start time, and uptime. Event-loop p99/max gauges make synchronous SQLite blocking visible. The endpoint is always bearer-authenticated and remains disabled until a token of at least 32 characters is configured:
...
The deletion ledger supports an S3-compatible bucket and a development/test-only filesystem backend. Every production deployment, including production self-hosting, must use a bucket outside the primary SQLite volume and every backup set that can restore it; production configuration rejects the filesystem backend. Each append-only immutable object contains a version, HMAC-SHA-256 pseudonym of the GitHub user id, deletion-attempt time, last-covered-backup expiry (`protectUntil`), random object id component, and an authentication MAC under the separately domain-separated ledger-entry purpose. It contains no raw GitHub id, login, token, or deleted-record inventory. Repeated requests are safe and may create multiple objects for one pseudonym. When startup reconciliation finds identity data to remove, it writes a fresh object before committing primary cleanup, so a delayed retry covers backups made after the original request. The HMAC key must be separate from the session secret and bucket credentials.

Configure the official S3 shape with `MULTAIPLAYER_RELAY_DELETION_LEDGER_S3_ENDPOINT`, `_BUCKET`, `_REGION`, `_ACCESS_KEY_ID`, `_SECRET_ACCESS_KEY`, `_URL_STYLE` (`path` or `virtual-host`), optional `_PREFIX`, the separate `MULTAIPLAYER_RELAY_DELETION_LEDGER_HMAC_KEY`, and `MULTAIPLAYER_RELAY_DELETION_LEDGER_PROTECTION_SECONDS`. The horizon must exceed the longest possible backup retention. The official configuration uses 7,776,000 seconds (90 days) around an 89-day maximum snapshot window; production config and the doctor reject a shorter value. A complete ledger is mandatory when `NODE_ENV=production`; an incomplete or unreachable ledger fails startup before the server listens.
