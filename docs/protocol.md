...
GitHub OAuth and ChatGPT authorization are out-of-band account protocols, not room or MLS events. GitHub Device Flow polling and access-token custody terminate in native Rust; the relay receives the token once over TLS for verify-then-discard identity bootstrap. Codex browser/device login terminates at the local app-server. Onboarding stores neither flow's transient identifiers. An invite join requires authenticated GitHub identity when relay auth is enabled, but does not require local Codex or ChatGPT authorization.
