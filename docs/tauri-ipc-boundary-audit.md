...
| `open_browser_view`               | none                | HTTPS/HTTP URL policy, room ID, room/project-derived child label, bounded navigation/tab session IDs, and bounded child-view geometry checked. Rust creates the child view with a nonpersistent data store, denies downloads, disables drag/drop handling, and installs the tested page guard. The frontend has no capability to create arbitrary webviews. |
| `navigate_browser_view`           | none                | Room/project-derived child label, exact active navigation/tab session, and a closed back/forward/reload action enum checked; operates only on that existing room-browser child. Reload uses the native WebView operation, and every resulting navigation is independently restricted to hosted HTTP/HTTPS URLs. |
| `browser_view_state`              | none                | Room/project-derived child label and exact active navigation/tab session checked; returns the native WebView's current validated HTTP/HTTPS URL so redirects and same-document navigation cannot leave stale browser provenance. |
| `position_browser_view`           | none                | Room/project-derived child label, exact active navigation/tab session, and finite, bounded geometry checked before moving only the existing room browser. The frontend has no direct webview positioning capability.                                               |
| `close_browser_view`              | none                | Room/project-derived child label and exact active navigation/tab session checked; closes only that existing room-browser child and ignores stale cleanup from replaced views. The frontend has no direct webview close capability.                                                                          |
...
| `local_preview_stop`           | local  | ID grammar and existing tunnel required; terminates and reaps the exact process before removing it from native state. Scoped ID is not caller authentication (IPC-03).                                          |
| `local_preview_stop_all`       | local  | Stops and reaps every locally owned Quick Tunnel process, retaining any process whose termination cannot be confirmed. Used for sign-out, account deletion, and app shutdown cleanup.                           |
