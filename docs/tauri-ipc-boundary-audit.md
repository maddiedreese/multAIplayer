...
| `open_browser_view`               | local               | HTTPS/HTTP URL policy, room ID and profile scope checked; profile directory is app-owned. A native callback denies downloads. A tested initialization guard denies page Clipboard API calls, file-input events, and drag/drop where WebKit permits script injection; its result reports installation rather than claiming platform enforcement. Caller-supplied title is not security authority. |
