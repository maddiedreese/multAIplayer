...
| `open_browser_view`               | local               | HTTPS/HTTP URL policy, room ID, profile scope, and bounded child-view geometry checked; profile directory is app-owned. Rust creates the child view, denies downloads, disables drag/drop handling, and installs the tested page guard. The frontend has no capability to create arbitrary webviews. |
| `position_browser_view`           | none                | Room/profile-derived child label and finite, bounded geometry checked before moving only the existing room browser. The frontend has no direct webview positioning capability.                                               |
| `close_browser_view`              | none                | Room/profile-derived child label checked; closes only that existing room-browser child. The frontend has no direct webview close capability.                                                                          |
| `reset_browser_profile`           | local               | Room/profile scope validated and mapped under app data; closes the matching view before deletion. Scoped identifier, not caller authentication (IPC-03).                                           |
