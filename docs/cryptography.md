...
The complete current project/Codex configuration is a host-authored `room.config` MLS application payload. Rust validates its exact schema, text bounds, revision, and emitting epoch, then saves the latest value in the SQLCipher application store before constructing the outbound PrivateMessage. That local record lets the host retry after Add, reconnect, or process restart; its SQLCipher wrapping key remains in the operating-system credential store. A receiver accepts only the active host's snapshot and only when its authenticated epoch/revision is newer. The relay sees only the opaque MLS message. Clearing local history retains this current configuration; **Forget on this device** deletes both the retained history and the configuration record.
