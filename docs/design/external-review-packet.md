...
The checked-in [synchronous-persistence soak](../benchmarks/relay-soak-2026-07-15.md) records request/publish p99, event-loop p99/max, SQLite-write histograms, WAL bounds, room/member cardinality, reloads, backup integrity, and resource use. It includes both the local checkpoint comparison and a 15-minute run on the production Railway volume using an isolated synthetic database. This is alpha regression and capacity evidence, not a hosted SLO or a claim about live user traffic.
