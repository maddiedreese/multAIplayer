...
test("createGitWorkflowApprovalPlan creates local-only branch and commit approvals", () => {
  ...
  assert.deepEqual(plan.approvals.map((approval) => approval.action), ["branch", "commit"]);
  assert.deepEqual(plan.approvals.flatMap((approval) => approval.commands), [
    "git switch -c 'feature/room-chat'",
    "git add -A",
    "git commit -m 'Add room chat polish'"
  ]);
});
...
test("createGitWorkflowApprovalPlan includes push and draft PR approval steps", () => {
  ...
  assert.deepEqual(plan.approvals.map((approval) => approval.action), [
    "branch",
    "commit",
    "push",
    "pull_request"
...
test("assertSafeBranchName rejects branch names that should not reach git commands", () => {
  ...
  assert.throws(
    () => assertSafeBranchName(`codex/${"x".repeat(maxGitBranchNameChars)}`),
    /Unsafe branch name/
  );
});
...
test("normalizeCommitMessage trims, bounds, and rejects empty messages", () => {
  ...
  assert.throws(
    () => normalizeCommitMessage("x".repeat(maxCommitMessageChars + 1)),
