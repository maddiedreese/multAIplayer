...
Stryker runs the deterministic focused crypto suite and excludes `properties.test.ts` from each mutant sandbox. The seeded property suite remains mandatory through the ordinary package and full repository tests; it is excluded only from the repeated mutation inner loop because the focused suite independently kills the complete governed mutant baseline. Any survivor after changing this composition is a test failure, not permission to lower the policy.
