...
`src/index.ts` and `src/inviteCapability.ts` additionally have whole-file zero-survivor gates so new code cannot land between marker regions without mutation coverage. Score floors for those files are recorded only after a complete measured run; the zero-survivor requirement applies immediately.
