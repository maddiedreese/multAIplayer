...
export function createThreadStartRequest(id: number, model = "gpt-5.3-codex"): JsonRpcRequest {
  return {
    method: "thread/start",
    id,
    params: { model }
...
export class CodexAppServerClient extends EventEmitter<CodexAppServerEvents> {
  ...
  async startThread(model = this.config.model ?? "gpt-5.3-codex"): Promise<CodexThreadStartResult> {
