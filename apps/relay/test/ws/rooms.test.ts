...
test("device retirement closes and removes only matching joined sockets", () => {
  const store = new InMemoryRelayStore();
  ...
  const retainedSocket = { close: () => assert.fail("unrelated socket must remain open") } as never;
  const retiredSession = {
    socket: retiredSocket,
    rateClientId: "retired",
    userId: "github:user",
  ...
  const sessions = new Map([
    [retiredSocket, retiredSession],
    [retainedSocket, retainedSession]
  ]);
  const manager = createRelayRoomSocketManager({
    store,
    roomSockets: new Map(),
    ...
    addTeamMember: () => {},
    scheduleStoreSave: () => {},
    send: (_socket, message) => sent.push(message.message),
    broadcast: () => {}
  });

  manager.revokeDeviceSessions("github:user", "retired-device");

  assert.deepEqual(closed, [[1008, "Device identity retired"]]);
  assert.match(sent[0] ?? "", /retired/i);
  assert.equal(sessions.has(retiredSocket), false);
  assert.equal(sessions.has(retainedSocket), true);
});
