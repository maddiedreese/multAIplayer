...
const historyMessages = {
  "room-alpha": [
    { id: "old-alpha", author: "Maddie", body: "older local history mention", attachments: [] }
  ],
  "room-beta": [
...
test("mergeSearchableMessages prefers live room messages over local history cache", () => {
  const merged = mergeSearchableMessages({
    "room-alpha": [
      { id: "live-alpha", author: "Maddie", body: "fresh loaded chat", attachments: [] }
    ],
    "room-empty": []
  }, historyMessages);
...
test("findSidebarMessageHits matches authors, message bodies, and attachment names", () => {
  ...
  assert.deepEqual(hits.map((hit) => `${hit.roomId}:${hit.message.id}`), ["room-beta:old-beta"]);
});
