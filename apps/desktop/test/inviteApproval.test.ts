...
test("invite request decisions require a pending request from the current room list", () => {
  const requests = [
    request,
    { ...request, id: "invite-request-2", status: "approved" as const }
  ];
  ...
  assert.equal(
    roomInviteRequestMessage(requests, "invite-request-2"),
    "Invite request is approved, not pending."
  );
  assert.equal(
    roomInviteRequestMessage(requests, "missing"),
    "Invite request is no longer available in this room."
  );
});
