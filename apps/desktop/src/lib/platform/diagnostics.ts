...
function sanitizeDiagnosticValue(value: unknown, depth: number, seen: WeakSet<object>): unknown {
  ...
  if (Array.isArray(value)) return sanitizeDiagnosticArray(descriptors, depth, seen);

  return sanitizeDiagnosticObject(descriptors, depth, seen);
}

function sanitizeDiagnosticArray(descriptors: PropertyDescriptorMap, depth: number, seen: WeakSet<object>) {
  ...
  for (let index = 0; index < itemCount; index += 1) {
    ...
    result.push(
      descriptor && "value" in descriptor ? sanitizeDiagnosticValue(descriptor.value, depth + 1, seen) : "[unavailable]"
    );
  }
...
function sanitizeDiagnosticObject(descriptors: PropertyDescriptorMap, depth: number, seen: WeakSet<object>) {
