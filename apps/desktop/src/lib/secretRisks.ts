...
export function detectTerminalCommandRisks(command: string): string[] {
  ...
  if (
    ...
    /(^|\s)(cat|less|more|tail|head|sed|awk|grep|rg|bat|open|code|vim|nano)\s+[^;&|]*(id_rsa|id_ed25519|\.npmrc|\.pypirc|credentials|secrets)\b/.test(lower) ||
    /(^|\s)(cat|less|more|tail|head|sed|awk|grep|rg|bat|open|code|vim|nano)\s+[^;&|]*\.(aws|config)\/credentials\b/.test(lower)
  ) {
    risks.add("Sensitive file access");
  }
  if (/(api[_-]?(key|token)|access[_-]?key(?:[_-]?id)?|access[_-]?token|auth[_-]?token|\btoken|secret|password|passwd|private[_-]?key)\s*[:=]/i.test(value)) {
  ...
  if (/(ghp_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{30,}|sk-[A-Za-z0-9_-]{20,}|-----BEGIN (RSA |OPENSSH |EC )?PRIVATE KEY-----)/.test(value)) {
    risks.add("Token or private key pattern");
  }
