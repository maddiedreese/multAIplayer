...
export async function transferMlsHost(
  ...
  return invokeNative("mls_transfer_host", { request: { roomId, nextHostLeaf, nextHostDeviceId, transferId } });
}
...
export function parseMlsAuthenticatedData(value: string): MlsAuthenticatedData | null {
  try {
    ...
    if (!validMlsAuthenticatedFields(parsed) || !hasOnlyMlsAuthenticatedKeys(parsed)) return null;
    return {
      version: 1,
      epoch: parsed.epoch!,
...
function validMlsAuthenticatedFields(parsed: Partial<MlsAuthenticatedData>): parsed is MlsAuthenticatedData {
  return (
    parsed.version === 1 &&
    Number.isSafeInteger(parsed.epoch) &&
    (parsed.epoch ?? -1) >= 0 &&
    nonemptyString(parsed.messageId) &&
    nonemptyString(parsed.teamId) &&
    nonemptyString(parsed.roomId) &&
    nonemptyString(parsed.kind) &&
    nonemptyString(parsed.senderUserId) &&
    nonemptyString(parsed.senderDeviceId) &&
...
function hasOnlyMlsAuthenticatedKeys(parsed: Partial<MlsAuthenticatedData>): boolean {
