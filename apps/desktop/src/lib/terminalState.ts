...
export function upsertTerminal<T extends RoomTerminalSnapshot>(
  current: T[],
  snapshot: T
): T[] {
...
export function mergeTerminalSnapshots<T extends PersistableTerminalSnapshot>(
  remembered: T[],
  live: T[]
): T[] {
...
export function terminalsForLocalHistory<T extends PersistableTerminalSnapshot>(
  terminals: T[]
): T[] {
  return terminals
    .map(terminalForLocalHistory)
