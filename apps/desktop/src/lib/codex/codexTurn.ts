...
import { detectSecretRisks } from "../security/secretRisks";
...
function addTextRiskFlags(
  ...
  if (
    /(?:\.\.[\\/]|\/(?:etc|home|Users)\/|outside (?:the )?(?:selected )?workspace|(?:read|open|copy|upload|exfiltrate)\b[^\n]{0,80}\b\.env\b)/i.test(
      text
    )
  ) {
    flags.push(createRiskFlag(source, "Workspace-boundary request"));
  }
