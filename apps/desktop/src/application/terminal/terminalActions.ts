...
} from "../../lib/terminal/terminalApproval";
import { canControlRoomTerminal, roomTerminalControlMessage } from "../../lib/terminal/terminalAccess";
import { nextShellTerminalName } from "../../lib/terminal/terminalUi";
...
export function createTerminalActions({
  ...
  function setTerminalErrorIfSelected(roomId: string, error: unknown) {
  ...
  async function approveTerminalRequest(request: TerminalCommandRequest) {
    ...
    try {
      ...
      setTerminalErrorIfSelected(roomId, error);
      setTerminalBusyForRoom(roomId, false);
      return;
    }
    ...
    try {
      ...
      publishTerminalResult(
        ...
        setTerminalErrorIfSelected(roomId, publishError);
      });
    } finally {
      setTerminalBusyForRoom(roomId, false);
    }
