...
export function createFileActions({
  ...
  async function saveSelectedFileContent(content: string) {
    ...
    await writeSelectedFileContent(room, path, content, selectedFile.content);
  }
  ...
  async function writeSelectedFileContent(
    ...
    setFileMessageForRoom(room.id, null);
    try {
      const saved = await writeProjectFile(room.projectPath, path, content, expectedContent);
  ...
  async function approveFileSaveRequest(request: WorkspaceFileSaveRequest) {
    ...
    const room = selectedRoom;
    const saved = await writeSelectedFileContent(room, request.path, request.nextContent, request.previousContent);
    if (!saved) return;
