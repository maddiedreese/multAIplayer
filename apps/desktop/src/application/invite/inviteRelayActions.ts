import type { ClientRoomRecord, MlsRelayMessage } from "@multaiplayer/protocol";
import {
  canActOnRoomInviteRequest,
  findRoomInviteRequest,
  roomInviteRequestMessage
} from "../../lib/invite/inviteApproval";
...
import { currentLocalIdentity, currentSelectedRoomContext } from "../workspace/selectedWorkspace";
import {
  consumeKeyPackage,
...
import { parseDirectedMlsInviteCiphertext } from "../../lib/invite/mlsInviteProtocol";
...
export function createInviteRelayActions(
  ...
  async function denyInviteJoinRequest(
    ...
    validated: NonNullable<ReturnType<typeof validatedRequests.get>>,
    roomId: string,
    deviceId: string
  ) {
    ...
    await markMlsPublishSucceeded(roomId, denial.outboxId);
    validatedRequests.delete(request.id);
    store.updateInviteRequestStatus(roomId, request.id, "denied");
    store.setInviteMessageForRoom(roomId, `Denied ${request.requester}'s join request.`);
  }
  ...
  async function decideInviteJoinRequest(request: InviteJoinRequest, status: InviteJoinRequest["status"]) {
    ...
    const requests = inviteRequestsForRoom(appStore, room.id);
    ...
    try {
      ...
      if (status === "denied") {
        await denyInviteJoinRequest(request, validated, room.id, deviceId);
        return;
      }
      await approveInviteJoinRequest(request, validated, context);
    } catch (error) {
  ...
  async function approveInviteJoinRequest(
    request: InviteJoinRequest,
    validated: NonNullable<ReturnType<typeof validatedRequests.get>>,
...
async function loadOrCreateInviteApproval(
  request: InviteJoinRequest,
  validated: NonNullable<ReturnType<typeof validatedRequests.get>>,
  room: ClientRoomRecord,
  deviceId: string
): Promise<{ approval: PendingInviteApproval; epoch: number }> {
  ...
  if (approval) return { approval, epoch };
  ...
  assertConsumedKeyPackageMatches(request, validated, consumed);
  ...
  return { approval, epoch };
}

function assertConsumedKeyPackageMatches(
  request: InviteJoinRequest,
  validated: NonNullable<ReturnType<typeof validatedRequests.get>>,
  consumed: Awaited<ReturnType<typeof consumeKeyPackage>>
) {
...
function inviteRequestsForRoom(store: AppStoreState, roomId: string) {
  return store.inviteByRoom[roomId]?.requests ?? [];
}
