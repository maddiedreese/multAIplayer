...
- `useLocalHistoryPersistence.ts` persists encrypted local history only after hydration succeeds. Shared snapshot construction keeps effect-driven saves and close-time all-room drains identical; native writes serialize per room, coalesce bursts, retain the newest bounded history, and replay blocked state when a destructive operation fails.
