...
Room traffic authenticates ciphertext together with its sender, room, event kind, timestamp, and key epoch. Invite links contain no room key: the active host validates a capability-authenticated device request before delivering the current epoch key, and member removal excludes removed devices from future epochs without claiming to erase content already delivered.

Start with [docs/using-the-app.md](docs/using-the-app.md) for the desktop features, [docs/product-architecture.md](docs/product-architecture.md) for the product model, [docs/codex-hosting.md](docs/codex-hosting.md) for how host-side Codex works, [docs/local-preview-sharing.md](docs/local-preview-sharing.md) for localhost preview tunnels, [docs/threat-model.md](docs/threat-model.md) for privacy boundaries, [docs/self-hosting.md](docs/self-hosting.md) for relay deployment, [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for embedded editor/terminal/browser attribution, and [docs/alpha-limitations.md](docs/alpha-limitations.md) for alpha limits. Contributors and maintainers can use [CONTRIBUTING.md](CONTRIBUTING.md), [SECURITY.md](SECURITY.md), [docs/release-hardening.md](docs/release-hardening.md), and [docs/alpha-release-readiness.md](docs/alpha-release-readiness.md).
